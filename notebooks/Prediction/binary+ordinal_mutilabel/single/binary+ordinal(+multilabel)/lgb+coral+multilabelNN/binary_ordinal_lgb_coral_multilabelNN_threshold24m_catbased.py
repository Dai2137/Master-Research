...
result_dir = r"D:\fujiwara\M\result\binary+ordinal_multilabel\single\binary+ordinal(+multilabel)\lgb+coral+multilabelNN\threshold_24m"
...
def predict_midpoint(model, X_df):
    ...
    model.eval()
    x_tensor = torch.tensor(X_df.values, dtype=torch.float32)
    label_to_midpoint = torch.tensor([31 * 0.5, 31 * 2.5, 31 * 6.5, 31 * 16.5], dtype=torch.float32)  # Midpoints for CORAL classes (<24 months)
...
# NOTE:
# - The target variable "days_until_next_category" is assumed to be created by the user's categorize_period(days) function.
# - For the binary task, we do NOT use raw days ("days_until_next") to create the threshold.
#
# Binary label (positive = <=24 months):
#   0: <=24 months  (categories 0..3)
#   1: >24 months   (categories 4..5; merged)
y_ordinal_raw = df['days_until_next_category'].values
y_binary = (y_ordinal_raw >= 4).astype(int)

# Ordinal label for evaluation / final output (5 classes):
#   0:<1m, 1:1-4m, 2:4-9m, 3:9-24m, 4:>24m (merged from raw categories 4..5)
y_ordinal = np.where(y_ordinal_raw <= 3, y_ordinal_raw, 4)

y_multilabel = df[[col for col in df.columns if col.startswith("on_day_reason_group_") and col.endswith("_next")]].values

# Ordinal class counts
num_ord_classes = len(np.unique(y_ordinal))  # should be 5
num_ord_classes_coral = 4                 # CORAL is trained only on <=24 months (classes 0..3)
coral_output_dim = num_ord_classes_coral - 1  # 3 binary tasks
...
# DMシミュレーション用設定
H_MONTHS_LIST = [1, 4, 9, 24]  # Threshold H (months)
H_TO_MAX_CATEGORY = {  # days_until_next_category のクラスとの対応
    1: 0,    # <1 month
    4: 1,    # 1-4 months
    9: 2,    # 4-9 months
    24: 3,   # 9-24 months
    # 120 is not used because >24 months is merged into one class
}
...
for seed in seeds:
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(X, y_ordinal)):
        ...
        # 0〜4のカテゴリに絞って件数確認
        counts_0to3 = train_df[train_df['days_until_next_category'].between(0, 3)]['days_until_next_category'].value_counts()

        # 最小件数のカテゴリを特定
        min_cat = counts_0to3.idxmin()
        target_counts = counts_0to3.min()

        # アンダーサンプリング対象カテゴリ（0〜4のうち最小以外）
        target_categories = [c for c in counts_0to3.index if c != min_cat]
        ...
        y_ord_train = y_ordinal[train_idx][mask_train]  # 0..3

        # 再登記されるものだけ使う（binary=0）
        mask_val = y_binary[val_idx] == 0
        X_val = X.iloc[val_idx].loc[mask_val]
        y_ord_val = y_ordinal[val_idx][mask_val]  # 0..3
        ...
        no_re_registration_class = label_names.index('>24 months')  # -> 4
...
with open(summary_all_path, "w", newline="", encoding="utf-8") as f:
    ...
    for metric in ["accuracy", "precision", "recall", "f1", "auc"]:
        ...
        writer.writerow(["Binary(positive:<=24 months, negative:>24 months)", "-", metric, mean, std])

    # 2. 順序回帰の評価指標
    for name in metric_names:
        values = [m[name] for m in all_metrics]
        mean, std = mean_std(values)
        writer.writerow(["Ordinal(days_until_next_5class)", "-", name, mean, std])
...
cm_bin_all_df = pd.DataFrame(cm_bin_all, index=["True: <=24 months", "True: >24 months"], columns=["Pred: <=24 months", "Pred: >24 months"])
...
cm_bin_all = confusion_matrix(all_y_bin_true, all_y_bin_pred, labels=[0, 1])
cm_bin_all_df = pd.DataFrame(cm_bin_all, index=["正解: 24ヶ月以内", "正解: 24ヶ月超"], columns=["予測: 24ヶ月以内", "予測: 24ヶ月超"])
...
label_indices = list(range(len(label_names)))  # -> [0, 1, 2, 3, 4]
...
plt.xlabel("Predicted Label")
plt.ylabel("True Label")
plt.title("Confusion Matrix (Ordinal Days Until Next Registration - All Folds)")
plt.tight_layout()
...
# 個別（カテゴリごと1枚）
for cat in range(len(label_names)):  # 0..4
