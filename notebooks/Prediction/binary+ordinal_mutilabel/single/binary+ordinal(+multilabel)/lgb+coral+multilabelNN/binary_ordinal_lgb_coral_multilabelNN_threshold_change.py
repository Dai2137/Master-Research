...
result_dir = r"D:\fujiwara\M\result\binary+ordinal_multilabel\single\binary+ordinal(+multilabel)\lgb+coral+multilabelNN\9_threshold"
...
THRESHOLD_DAYS = 31 * 9  # 9 months
# 0: <=9 months（近い）, 1: >9 months（遠い）
y_binary = (df['days_until_next'] > THRESHOLD_DAYS).astype(int).values
...
cm_bin_all_df = pd.DataFrame(
    cm_bin_all,
    index=["True: <=24 months", "True: >24 months"],
    columns=["Pred: <=24 months", "Pred: >24 months"]
)
...
cm_bin_all_df = pd.DataFrame(
    cm_bin_all,
    index=["正解: 9ヶ月以内", "正解: 9ヶ月超"],
    columns=["予測: 9ヶ月以内", "予測: 9ヶ月超"]
)
