...
result_dir = r"D:\fujiwara\M\result\binary+ordinal_multilabel\single\binary+ordinal(+multilabel)\lgb+coral+multilabelNN\threshold_9m"
...
def predict_midpoint(model, X_df):
    ...
    model.eval()
    x_tensor = torch.tensor(X_df.values, dtype=torch.float32)
    label_to_midpoint = torch.tensor([31 * 0.5, 31 * 2.5, 31 * 6.5], dtype=torch.float32)  # Midpoints for CORAL classes (<24 months)
...
num_ord_classes_coral = 3                 # CORAL is trained only on <=9 months (classes 0..2)
coral_output_dim = num_ord_classes_coral - 1  # 2 binary tasks
...
for seed in seeds:
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(X, y_ordinal)):
        ...
        no_re_registration_class = label_names.index('>9 months')  # -> 3
...
with open(summary_all_path, "w", newline="", encoding="utf-8") as f:
    ...
    for metric in ["accuracy", "precision", "recall", "f1", "auc"]:
        ...
        writer.writerow(["Binary(positive:<=9 months, negative:>9 months)", "-", metric, mean, std])
