...
# Path
# =========================
result_dir = r"D:\fujiwara\M\result\binary+ordinal_multilabel\multi\MoE"
...
class Expert(nn.Module):
    def __init__(self, input_dim, hidden_dims=(256, 128, 64), dropout=0.2):
        ...
        # 64-dim residual refinement
        self.res = ResidualMLPBlock(d3, dropout=dropout)
...
class MMoE(nn.Module):
    ...
    - task-specific gates
    """
    def __init__(self, input_dim, num_experts=6, expert_hidden=(256, 128, 64), dropout=0.2):
        super().__init__()
        self.num_experts = num_experts
        self.experts = nn.ModuleList([
            Expert(input_dim, hidden_dims=expert_hidden, dropout=dropout)
            for _ in range(num_experts)
        ])
        # gates: use raw input to decide mixing
        self.gate_ord = nn.Linear(input_dim, num_experts)
        self.gate_multi = nn.Linear(input_dim, num_experts)

    def forward(self, x):
        # expert outputs: (N, E, 64)
        expert_outs = torch.stack([ex(x) for ex in self.experts], dim=1)

        w_ord = F.softmax(self.gate_ord(x), dim=1).unsqueeze(-1)    # (N, E, 1)
        w_mul = F.softmax(self.gate_multi(x), dim=1).unsqueeze(-1)  # (N, E, 1)

        h_ord = (expert_outs * w_ord).sum(dim=1)   # (N, 64)
        h_mul = (expert_outs * w_mul).sum(dim=1)   # (N, 64)
        return h_ord, h_mul

class CoralHead(nn.Module):
    """
    CORAL head with ordered bias
    input: (N, d)
    output: (N, K-1) cumulative probs
    """
    def __init__(self, input_dim, num_classes):
        ...
        # shared weight + ordered bias
        self.shared_weight = nn.Parameter(torch.randn(input_dim))
        self.raw_bias = nn.Parameter(torch.zeros(num_classes - 1))

    def forward(self, h):
        # logits base: (N,)
        base = (h * self.shared_weight).sum(dim=1, keepdim=True)  # (N, 1)

        ordered_bias = torch.cumsum(F.softplus(self.raw_bias), dim=0)  # (K-1,)
        logits = base + ordered_bias  # (N, K-1) broadcasting
        probs = torch.sigmoid(logits)
        return probs

class TaskTower(nn.Module):
    def __init__(self, in_dim, mid_dim=64, out_dim=32, dropout=0.25):
        super().__init__()
        self.fc_in = nn.Linear(in_dim, mid_dim)
        self.bn_in = nn.BatchNorm1d(mid_dim)
        self.drop = nn.Dropout(dropout)
        self.res1 = ResidualMLPBlock(mid_dim, dropout=dropout)
        self.fc_out = nn.Linear(mid_dim, out_dim)
...
class MultiTaskMMoE_CORAL(nn.Module):
    """
    MMoE + deeper task towers
    - ordinal: CORAL
    - multilabel: sigmoid
    """
    def __init__(self, input_dim, num_ord_classes, num_multilabels,
                 num_experts=6, expert_hidden=(256, 128, 64),
                 dropout_expert=0.2, dropout_tower=0.25):
        super().__init__()
        self.mmoe = MMoE(
            input_dim=input_dim,
        ...
        # task towers
        self.tower_ord = TaskTower(64, mid_dim=64, out_dim=32, dropout=dropout_tower)
        self.tower_mul = TaskTower(64, mid_dim=64, out_dim=32, dropout=dropout_tower)

        # heads
        self.coral_head = CoralHead(input_dim=32, num_classes=num_ord_classes)
        self.mul_head = nn.Linear(32, num_multilabels)

    def forward(self, x):
        h_ord, h_mul = self.mmoe(x)
        z_ord = self.tower_ord(h_ord)
        z_mul = self.tower_mul(h_mul)

        probs_ord = self.coral_head(z_ord)                # (N, K-1)
        probs_mul = torch.sigmoid(self.mul_head(z_mul))   # (N, L)
        return probs_ord, probs_mul
...
# マルチタスク損失の重み（file1の 1.66 を踏襲）
lambda_multilabel = 1.66
...
for seed in seeds:
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(X, y_ordinal)):
        ...
        model_mt = MultiTaskMMoE_CORAL(
            input_dim=X.shape[1],
            num_ord_classes=num_ord_classes_coral,
            num_multilabels=y_multilabel.shape[1],
            num_experts=6,
            expert_hidden=(256, 128, 64),
            dropout_expert=0.2,
            dropout_tower=0.25
        )
