...
# Path
# =========================
result_dir = r"D:\fujiwara\M\result\binary+ordinal_multilabel\multi\purpose_prior"
...
class MultiLabelThenCORAL(nn.Module):
    """
    1) shared trunk
    2) multilabel head -> p_multi
    3) ordinal head takes [shared, p_multi] as features -> CORAL probs (K-1)
    """
    def __init__(self, input_dim: int, num_ord_classes: int, num_multilabels: int):
        super().__init__()
        self.num_ord_classes = num_ord_classes
        self.num_multilabels = num_multilabels
        ...
        # ---- multilabel head (先に推定) ----
        self.multi_specific = nn.Linear(32, 16)
        self.multi_out = nn.Linear(16, num_multilabels)

        # ---- ordinal head (multi-label確率を特徴量として使用) ----
        # concat: 32 + L -> 16
        self.ord_fuse = nn.Linear(32 + num_multilabels, 16)

        # CORAL params
        self.shared_weight = nn.Parameter(torch.randn(16))          # (16,)
        self.raw_bias = nn.Parameter(torch.zeros(num_ord_classes - 1))  # (K-1,)

    def forward(self, x, detach_multilabel_for_ordinal: bool = False):
        """
        detach_multilabel_for_ordinal:
          True  -> ordinal loss backprop が multilabel head に流れない（安定したいとき）
          False -> end-to-end（ordinalがmultilabelを引っ張る）
        """
        # shared
        h = self.relu(self.fc1(x))
        h_shared = self.relu(self.fc2(h))  # (N, 32)

        # multilabel
        h_multi = self.relu(self.multi_specific(h_shared))          # (N, 16)
        p_multi = torch.sigmoid(self.multi_out(h_multi))            # (N, L)

        # use multilabel probs as features for ordinal
        p_multi_feat = p_multi.detach() if detach_multilabel_for_ordinal else p_multi
        h_fused = torch.cat([h_shared, p_multi_feat], dim=1)        # (N, 32+L)
        h_ord = self.relu(self.ord_fuse(h_fused))                   # (N, 16)

        # CORAL logits (N, 1) + (K-1)
        base = (h_ord @ self.shared_weight).unsqueeze(1)            # (N, 1)
        ordered_bias = torch.cumsum(F.softplus(self.raw_bias), dim=0)  # (K-1,)
        logits = base + ordered_bias                                # (N, K-1)
