...
# Path
# =========================
result_dir = r"D:\fujiwara\M\result\binary+ordinal_multilabel\multi\simple"
...
# =========================
# Model (MultiTask CORAL)
#  - shared trunk: 64 -> 32
#  - ordinal head: 32 -> 16 -> CORAL logits (shared weight + ordered bias)
#  - multilabel head: 32 -> 16 -> sigmoid
# =========================
class CoralOrdinalBackbone(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        # ordinal specific
        self.ord_specific = nn.Linear(32, 16)
    ...
    def forward(self, x):
        ...
        shared = self.relu(self.fc2(x))          # (N, 32)
        x_ord = self.relu(self.ord_specific(shared))  # (N, 16)

        logits = x_ord @ self.shared_weight      # (N,)
        logits = logits.unsqueeze(1)             # (N, 1)

        ordered_bias = torch.cumsum(F.softplus(self.raw_bias), dim=0)  # (K-1,)
        logits = logits + ordered_bias           # broadcast to (N, K-1)

        probs_ord = torch.sigmoid(logits)
        return probs_ord, shared

class MultiTaskCORAL(nn.Module):
    def __init__(self, input_dim, num_ord_classes, num_multilabels):
        super().__init__()
        self.coral = CoralOrdinalBackbone(input_dim, num_ord_classes)
    ...
    def forward(self, x):
        probs_ord, shared = self.coral(x)
        x_multi = self.relu(self.multi_specific(shared))
