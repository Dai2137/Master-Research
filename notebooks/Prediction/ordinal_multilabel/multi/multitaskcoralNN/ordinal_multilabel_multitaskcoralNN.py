...
result_dir = r"D:\fujiwara\M\result\ordinal_multilabel\multi\multitaskcoralNN"
...
for seed in seeds:
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(X, y_ordinal)):
        ...
        with torch.no_grad():
            probs_ord_all, probs_multi_all = model(X_test_tensor)
...
print("統合評価の保存フェーズ開始")
# ========= 統合評価CSVの出力（完全版） =========
summary_all_path = os.path.join(result_dir, "metrics_all_summary_multitaskcoralNN.csv")
...
plt.tight_layout()
plt.savefig(os.path.join(result_dir, "confusion_matrix_ordinal_multitaskcoralNN.png"))
plt.close()
