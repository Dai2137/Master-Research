...
result_dir = r"D:\fujiwara\M\result\ordinal_multilabel\single\ordinal\olr\no_5_undersample\only_reregister_training"
...
for seed in seeds:
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(X, y_ordinal)):
        ...
        # olr の出力（二値タスクごとの評価）
        # OrderedModel 出力（各クラス確率）
        class_probs = probs_ord.to_numpy()  # shape: (n_samples, K_actual)
        K_actual = class_probs.shape[1]     # 実際のクラス数
        olr_output_dim_actual = K_actual - 1

        # 各クラス確率 → 累積確率（y <= k の確率）
        cum_probs = np.cumsum(class_probs, axis=1)[:, :-1]  # shape: (n_samples, K_actual-1)

        # 累積ラベル生成（y <= k）※k=0..K-2 だけ作る
        y_ord_bin_targets = (y_ord_test[:, None] <= np.arange(olr_output_dim_actual)[None, :]).astype(int)

        # 0.5 閾値で二値予測
        y_ord_bin_preds = (cum_probs > 0.5).astype(int)

        print("\n[olr内部の二値タスクごとの評価指標]")
        for k in range(olr_output_dim_actual):
            acc_k = accuracy_score(y_ord_bin_targets[:, k], y_ord_bin_preds[:, k])
            precision_k = precision_score(y_ord_bin_targets[:, k], y_ord_bin_preds[:, k], zero_division=0)
            recall_k = recall_score(y_ord_bin_targets[:, k], y_ord_bin_preds[:, k], zero_division=0)
            f1_k = f1_score(y_ord_bin_targets[:, k], y_ord_bin_preds[:, k], zero_division=0)
            ...
            label_k = label_names[k] if k < len(label_names) else f"k={k}"
            print(f"Task y <= '{label_k}': Acc={acc_k:.4f}, Precision={precision_k:.4f}, Recall={recall_k:.4f}, F1={f1_k:.4f}, AUC={auc_k:.4f}")
...
print("統合評価の保存フェーズ開始")
# ========= 統合評価CSVの出力（完全版） =========
summary_all_path = os.path.join(result_dir, "metrics_all_summary_olr_no_5_undersample.csv")
