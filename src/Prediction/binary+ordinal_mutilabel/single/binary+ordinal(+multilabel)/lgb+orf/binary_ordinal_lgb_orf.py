...
# ===== containers for ordinal metrics (days) =====
all_metrics_argmax = []
all_metrics_expected = []

# ===== containers for scatter / pearson corr (days) =====
all_true_days = []
...
for seed in seeds:
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(X, y_ordinal)):
        ...
        # ===== Expected value days (ORF) =====
        # 初期値：再登記されないと予測されたものは class 5 の中央値
        y_pred_days_expected = np.full(
            shape=len(y_ord_test),
            fill_value=label_to_midpoint[no_re_registration_class],
            dtype=float
        )

        # 再登記されると予測されたもののみ ORF の確率期待値を使う
        if mask_bin0.sum() > 0:
            class_probs = probs_ord_masked_df.values  # shape: (n_masked, 5) for classes 0..4
            midpoint_vec = np.array(
                [label_to_midpoint[c] for c in range(num_ord_classes_orf)],
                dtype=float
            )
            exp_days_masked = (class_probs * midpoint_vec[None, :]).sum(axis=1)
            y_pred_days_expected[mask_bin0] = exp_days_masked
        ...
        # 初期値：再登記されない予測は class 5 の中央値
        y_pred_days_expected = np.full_like(
            y_pred_days,
            fill_value=float(label_to_midpoint[no_re_registration_class]),
            dtype=float
        )

        # 再登記される（binary=0）と予測されたサンプルのみ ORF期待値
        if mask_bin0.sum() > 0:
            # ORF の predict_proba で得た確率（classes 0..4 のみ）
            # ここはあなたの変数名に合わせて：
            #   probs_ord_masked_df: shape (n_masked, 5) の DataFrame
            class_probs = probs_ord_masked_df.values.astype(float)

            midpoint_vec_0to4 = np.array(
                [label_to_midpoint[c] for c in range(num_ord_classes_orf)],
                dtype=float
            )
            exp_days_masked = (class_probs * midpoint_vec_0to4[None, :]).sum(axis=1)
            y_pred_days_expected[mask_bin0] = exp_days_masked
        ...
        print(f"[Ordinal (Argmax->Midpoint Days)] MAE={mae:.4f}, RMSE={rmse:.4f}, Corr={corr:.4f}")
        print(f"[Ordinal (Expected Days)]        MAE={mae_exp:.4f}, RMSE={rmse_exp:.4f}, Corr={corr_exp:.4f}")
...
with open(summary_all_path, "w", newline="", encoding="utf-8") as f:
    ...
    writer.writerow(["Ordinal(ExpectedDays)", "-", "Pearson_corr_days", corr_exp, "-", f"p={pval_exp}"])
...
plt.title("True vs Predicted Days (Expected, ORF)")
plt.tight_layout()
